...
class KintoClient {
  ...
  public collection<T>(collectionName: string): KintoCollection<T> {
    return {
      add: (record: T) => {
        return this.client
          .create<KintoResult<T>>(`${collectionName}/records`, { data: record })
          .pipe(map((res: KintoResult<T>) => res.data));
      },
      all: () => {
        return this.client
          .get<KintoResult<T[]>>(`${collectionName}/records`)
          .pipe(map((res: KintoResult<T[]>) => res.data));
      },
      ...
      one: (recordId: string) => {
        return this.client
          .get<KintoResult<T>>(`${collectionName}/records/${recordId}`)
          .pipe(map((res: KintoResult<T>) => res.data));
      },
      update: (recordId: string, record: T) => {
        return this.client
          .update<KintoResult<T>>(`${collectionName}/records/${recordId}`, {
            data: record
          })
          .pipe(map((res: KintoResult<T>) => res.data));
      },
      ...
      search: (filter: string) => {
        return this.client
          .get<KintoResult<T[]>>(`${collectionName}/records?${filter}`)
          .pipe(map((res: KintoResult<T[]>) => (res.data ? res.data : [])));
      }
    };
  }
}
