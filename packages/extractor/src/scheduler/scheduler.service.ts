...
export const handleScheduler = (cron: string, scheduler: string, process: (start: number, end: number) => Observable<any>) => {
    schedule(cron, () => {
        ...
        const runScheduler$ = synchroHistoryService.getSynchroHistory(scheduler).pipe(
            tap(syncHistory => logger.info(`[Scheduler] ${scheduler} - ${new Date(syncHistory.last_synchro)} > ${endDate}`)),
            exhaustMap(syncHistory => process(syncHistory.last_synchro, end)),
        );
        runScheduler$.pipe(
            takeLast(1),
