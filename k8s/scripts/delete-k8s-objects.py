...
def get_active_k8s_tags():
  ...
  for active_k8s_tag in raw_k8s_tag_list.decode('utf-8').replace('<no value>','').split('wif-bo-'):
...
def delete_k8s_object(label):
  ...
  for k8s_object in k8s_object_list:
    command_to_delete_k8s_object = ('kubectl delete '+ k8s_object +' --selector branch=wif-bo-'+label)
...
if __name__ == '__main__':
  for k8s_tag_to_delete in get_k8s_tag_to_delete(get_active_k8s_tags(), get_active_branches()):
    ...
    print('k8s objects with label branch=wif-bo-'+k8s_tag_to_delete+' have been deleted')
